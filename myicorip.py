...
<head>
  ...
  <title>{{ title or 'Easy Order' }}</title>
...
<nav class="navbar navbar-expand-lg bg-light mb-4">
  <div class="container-fluid">
    <div class="d-flex flex-column">
      <a class="navbar-brand" href="{{ url_for('home') }}">Easy Order</a>
      {% if session.get('user') %}
        <span class="navbar-text text-muted small">Ciao {{ session['user']['username'] }}</span>
      {% endif %}
    </div>
    <div class="d-flex">
      {% if session.get('user') %}
        <a class="btn btn-outline-primary me-2" href="{{ url_for('cart') }}">Ordine</a>
...
{% extends 'layout.html' %}
{% block body %}
<h1 class="h4 mb-3">Ricerca articoli</h1>
<form method="post" action="{{ url_for('search') }}" class="row g-2 mb-3">
  <div class="col-sm-8">
    <input name="q" value="{{q or ''}}" class="form-control" placeholder="Descrizione, Codice o EAN">
  </div>
  ...
  {% if results %}
    <div class="table-responsive">
      <table class="table table-striped table-sm align-middle">
        <thead><tr><th>Codice</th><th>Descrizione</th><th class="text-center" style="width:280px">Q.tà</th></tr></thead>
        <tbody>
        {% for r in results %}
          <tr>
            <td>{{ r.codart }}</td>
            <td>{{ r.descrizione }}</td>
            <td>
              <form method="post" action="{{ url_for('add_to_cart') }}" class="d-flex gap-2 justify-content-end">
                ...
                <input name="note" maxlength="50" class="form-control form-control-sm" placeholder="Note (max 50)" style="max-width:160px">
...
{% extends 'layout.html' %}
{% block body %}
<h1 class="h4 mb-3">Ordine</h1>
{% if cart %}
  <form method="post" action="{{ url_for('update_cart') }}">
    <div class="table-responsive">
      <table class="table table-striped table-sm align-middle">
        <thead><tr><th>Codice</th><th>Descrizione</th><th>EAN</th><th class="text-center" style="width:220px">Q.tà</th><th style="width:80px"></th></tr></thead>
        <tbody>
          {% for codart, item in cart.items() %}
          <tr>
            <td>{{ codart }}</td>
            <td>
              {{ item.descr }}
              {% if item.pack==0 %}
                <div class="small text-warning">⚠ Pack non configurato (step 1)</div>
              {% endif %}
              {% if item.note %}
                <div class="small text-muted">Note: {{ item.note }}</div>
              {% endif %}
            </td>
            <td>{{ item.ean }}</td>
            <td>
              <input name="qty_{{codart}}" type="number"
                     min="{{ item.pack if item.pack>0 else 1 }}"
                     step="{{ item.pack if item.pack>0 else 1 }}"
                     value="{{ item.qty }}"
            ...
            <td class="text-end">
              <form method="post" action="{{ url_for('remove_item') }}">
                ...
                <button class="btn btn-sm btn-outline-danger" title="Rimuovi">✕</button>
              </form>
            </td>
          </tr>
          {% endfor %}
    ...
    <div class="d-flex gap-2">
      <button class="btn btn-success" type="submit">Aggiorna quantità</button>
      <a class="btn btn-outline-danger" href="{{ url_for('clear_cart') }}">Svuota</a>
      <a class="btn btn-secondary" href="{{ url_for('home') }}">Continua</a>
      <a class="btn btn-primary" href="{{ url_for('export_csv') }}">Esporta CSV</a>
    </div>
  </form>
{% else %}
  <div class="alert alert-info">Ordine vuoto</div>
  <a class="btn btn-secondary" href="{{ url_for('home') }}">Torna alla ricerca</a>
{% endif %}
{% endblock %}
""",
}

# Loader: prima file su disco (override), poi fallback in memoria
